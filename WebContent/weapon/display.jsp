...
<body>
	<% 	
		...
		if(request.getSession().getAttribute("current_page")!=null){
			current_page = Integer.parseInt(request.getSession().getAttribute("current_page").toString());
		}else{
			current_page = 1;
		}
		//获取总页面数，用于动态生成页面跳转标签
		page_num = Integer.parseInt(request.getSession().getAttribute("page_num").toString());
		
		List<Weapon> weaponList = (List<Weapon>)request.getSession().getAttribute("weaponList");
	%>
    <!--头部开始-->
                        ...
                        <% if(weaponList != null){ %>
			                <% for(Weapon weapon:weaponList){%>
			                    <div style="width:100%; color: #eee;text-align: center;">
			                        ...
			                        <div  style="display:inline-block; float: left;width: 10.5%;">|</div>		                        
			                        <div  style="display:inline-block; width: 15.5%;float: left;overflow: hidden;">
			                        	
			                        	<div style="float:left;width: 45%">
			                        		<a style="color:#a00;" href="weapon_detail.jsp?model=<%=weapon.getModel() %>">详情</a>
			                        	</div>
			                ...
			                <%}%>
            			<%}%>
            			<div style="width:96%;color: #FFF"><%=" 本次搜索共统计出 " +request.getSession().getAttribute("count")+" 条数据!" %></div>
                      	<!-- 动态生成页面跳转标签 -->
            			<div style="width:96%; color: #eee;text-align: center;top:20px;position:relative;">
            				<a href="../weapon_searchServlet?current_page=<%=current_page>1 ? current_page-1:1%>"><%="< "%></a>
            				<%for(int i=1;i<=page_num;i++){ %>
            					<%if (i == current_page){ %>
            						<a style="color:#a00" href="../weapon_searchServlet?current_page=<%=i%>"><%=" "+i+" "%></a>
            					<%}else{ %>
	            					<a href="../weapon_searchServlet?current_page=<%=i%>"><%=" "+i+" "%></a>
                      			<%}%>
                      		<%}%>
                      		<a href="../weapon_searchServlet?current_page=<%=current_page<page_num ? current_page+1:page_num%>"><%=" >"%></a>
            			</div>
                    </div>
                </div>
            </div>
