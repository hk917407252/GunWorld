...
<body>
	<% 	request.setCharacterEncoding("utf-8");
		...
		Weapon weapon =null;
		weapon = sfr.loadByModel(request.getParameter("model")) ;%>
    <!--头部开始-->
        ...
        <div class="wp-wrapper" style="height: 920px;">
            <div class="wp-inner clearfix" style="height: 920px;">
                ...
                <div class="news clearfix">
                    <div frag="更新武器信息模块"style="color:#eee;" class="font">
                     	<form action="../weapon_updateServlet" style="text-align: center;" method="post">
                     		<%String m = request.getParameter("model"); 
                     			request.getSession().setAttribute("model", m);%>
                     		名称：<input type="text" name="name" value="<%=weapon.getName()%>"><br>
                     		口径：<input type="text" name="caliber" value="<%=weapon.getCaliber()%>"><br>
                     		类型：<input type="text" name="type" value="<%=weapon.getType()%>"><br>
                     		种类：<input type="text" name="variety" value="<%=weapon.getVariety()%>"><br>
                     		路径：<input type="text" name="route" value="<%=weapon.getRoute()%>"><br>
                     		详情：<textarea name="detail" clos="25" rows="8" value=""><%=weapon.getDetail()%></textarea><br>
                     		<c:if test="${not empty error_input}">
								<span style="color:#ff0000">** ${error_input}**</span><br>
							</c:if>
                     		<input type="submit" value="确认">
                     		<input type="reset" value="重置">
                     		<a style="color:#ff0000" href="weapon_detail.jsp?model=<%=m%>">返回</a><br>
                     	</form>
                    </div>
                </div>
            </div>
